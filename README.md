...
| `PATCH`  | `/comments/:commentId`            | Обновить текст и/или `resolved`                |
| `DELETE` | `/comments/:commentId`            | Удалить комментарий (и ответы, если корневой)  |

Тело создания корневого комментария (`POST`):
...
Тело ответа на комментарий (`POST`):

```json
{
  "text": "Текст ответа",
  "parentCommentId": "cmt-101"
}
```
...
Координаты `relX` / `relY` — доля ширины и высоты **видимой страницы** (0…1), чтобы позиция не зависела от масштаба. Ответы наследуют якорь родителя.
